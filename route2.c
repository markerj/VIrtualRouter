	...
	//have the list, loop over the list
  	...
  	while(1)
  	{
    	...
    	else if((struct ethheader *)buf->eth_type == 0x800))
    	{
    		...
    		if(icmphdr->type == 8)
    		{
    			printf("Received ICMP ECHO request from %s (code: %u  id: %u  seq: %u)", inet_ntoa(*(struct in_addr *)&iphdr->src_ip),
    		...
    		icmphdrsend->checksum = in_chksum((char *)icmphdrsend, (1500 - sizeof (struct ethheader) - sizeof(struct ipheader)));
